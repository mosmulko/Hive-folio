...
function subheaderThrottle() {
    let scrolling = false;
    document.addEventListener('scroll', function(){scrolling = true}, { passive: true });
    setInterval(() => {
        if (scrolling) {
            scrolling = false;
            subHeaderChange()
...
const gallery = function() {
    ...
    projects.addImageListeners = function() {
        this.nextBtns.forEach(e => {
            e.addEventListener('click', function(){changeDisplayedImage(e, 1)})
        });
...
const contact = function () {
    ...
    const submit = cForm.submit;
    const inputs = cForm.inputs;
    ...
    function resetPlaceholder(target) {
        const placeholders = cForm.placeholders;
